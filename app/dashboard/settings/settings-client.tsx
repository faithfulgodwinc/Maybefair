...
interface SettingsClientProps {
    user: any; // Using any for simplicity with Supabase user object, could be typed stricter
}
...
export function SettingsClient({ user }: SettingsClientProps) {
    ...
    // Mock settings state (could be persisted to DB in future)
    ...
    const displayName = user.user_metadata?.full_name || user.user_metadata?.name || 'Executive User';
                ...
                <Card className="h-full">
                    ...
                    <CardContent className="space-y-6">
                        <div className="flex items-center gap-5 p-4 bg-secondary/20 rounded-lg border border-border/50">
                            ...
                            <div>
                                <h3 className="text-base font-medium tracking-tight">{displayName}</h3>
